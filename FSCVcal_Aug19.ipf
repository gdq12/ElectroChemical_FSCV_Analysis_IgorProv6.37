...
static function draw()
	...
	Button TCbut,pos={188,1},size={125,20},proc=FSCVcal#CalcOx,title="Graph Timecourse"
	...
	SetVariable con4Var,value= root:Calibration:con4da
		//needed to identify title for SetVariable so it would be possible to input known DA concentration for Calibration function 
	...
	Button SlopeCalcBut,pos={1065,486},size={50,20},disable=1,proc=FSCVcal#Calibration
		//made mistake and buy accident made xtra SlopeCalcBut, was not possible to disable this button no matter how many times deleted, so left in code and disabled 
	TitleBox SEMcalc,pos={970,530},size={12,24},title=" "
	TitleBox SEMcalc,labelBack=(65535,65535,65535),fSize=12,frame=2
	Button RedimBut,pos={7,1},size={150,20},proc=FSCVcal#Redim10Hz,title="Redimension Waves"
	Display/W=(7,23,1185,330)/HOST=# 
...
static function init()
	Button RedimBut, proc=FSCVcal#Redim10Hz, win=Calibrator
...
static function Redim10Hz(waves): ButtonControl
	...
	for(i=2;i<(ItemsinList(theWaveList, ";"))*2+1;i+=2)
		string NewName1="Cal_"+ num2str(i)
...
static function CalcOx(oxidation): ButtonControl
	...
	string theWaveList1=WaveList("Cal*",  ";", "DIMS:1")
...
//for baseline wave
	...
	variable b=pcsr(B, "Calibrator#Timecourse")+1 //wrote like this because when examined list string, the desired waves were recalled 
		//variables a nad b set to cursor position so the for loop can recall the wave names from start to finish based in the cursor position in the timecourse wave
...
//for second DA concentration wave 
	...
	for(q=e; q<f; q+=1)
		...
		DA2LineList=AddListItem(theWaveNameDA2, DA1LineList, "; ")
	endfor
...
//for third DA concentration wave
	...
	for(q=g; q<h; q+=1)
		...
		DA3LineList=AddListItem(theWaveNameDA3, DA1LineList, "; ")
	endfor
...
//for fourth DA concetration wave
	...
	for(q=i; q<j; q+=1)
		...
		DA4LineList=AddListItem(theWaveNameDA4, DA1LineList, "; ")
	endfor
